...
const BRIDGED_OVERLAY_FIELDS: SpecRegistryField[] = [
  {
    ...
    mandatory_flag_by_use_case: 'Conditional Mandatory (Credit note)',
    pint_ae_cardinality: '0..1',
    data_type: 'Code',
    format_pattern: 'Credit note reason code from the governed UAE credit note reason code list',
    validation_logic: 'Required when Invoice type code indicates a credit note.',
    derivation_logic: 'Provided by ERP input; no ASP derivation.',
    error_message_text: 'Credit note reason code is required for credit note documents.',
    ubl_xml_path: 'cac:DiscrepancyResponse/cbc:ResponseCode',
    mls_relevance: 'medium',
    vat_law_status: 'Conditional',
    data_responsibility: 'ERP (Corner 1)',
  },
  {
    dr_id: 'SYS-CN-REASON-TEXT',
    business_term: 'Credit note reason text',
    category: 'conditional_header',
    mandatory_flag_by_use_case: 'Conditional Recommended (Credit note narrative context)',
    pint_ae_cardinality: '0..1',
    data_type: 'String',
    format_pattern: 'Free-text reason or description supporting the credit note adjustment',
    validation_logic: 'Recommended for credit note readability when the source system captures a narrative alongside the reason code.',
    derivation_logic: 'Provided by ERP input; no ASP derivation.',
    error_message_text: 'Credit note reason text should be available for regulator-friendly and audit-friendly credit note context.',
    ubl_xml_path: 'cac:DiscrepancyResponse/cbc:Description',
    mls_relevance: 'medium',
    vat_law_status: 'Conditional',
    data_responsibility: 'ERP (Corner 1)',
  },
  {
    dr_id: 'IBG-03',
    business_term: 'Preceding invoice reference group',
    category: 'conditional_header_group',
    mandatory_flag_by_use_case: 'Conditional Mandatory (Credit note except VD reason code)',
    pint_ae_cardinality: '0..1',
    data_type: 'Group',
    format_pattern: 'Preceding invoice identifier with optional preceding invoice issue date',
    validation_logic: 'Required for credit notes unless the credit note reason code is VD.',
    derivation_logic: 'Grouped from preceding invoice reference identifier and preceding invoice issue date.',
    error_message_text: 'Preceding invoice context is required for credit notes unless the reason code waives the reference requirement.',
    ubl_xml_path: 'cac:BillingReference/cac:InvoiceDocumentReference',
    mls_relevance: 'medium',
    vat_law_status: 'Conditional',
    data_responsibility: 'ERP (Corner 1)',
  },
  {
    dr_id: 'IBT-025',
    business_term: 'Preceding invoice reference',
    category: 'conditional_header',
    mandatory_flag_by_use_case: 'Conditional Mandatory (Credit note except VD reason code)',
    pint_ae_cardinality: '0..1',
    data_type: 'String',
    format_pattern: 'Original invoice identifier',
    validation_logic: 'Required for credit notes unless the credit note reason code is VD.',
    derivation_logic: 'Provided by ERP input; no ASP derivation.',
    error_message_text: 'Preceding invoice reference is required for credit notes unless the reason code is VD.',
    ubl_xml_path: 'cac:BillingReference/cac:InvoiceDocumentReference/cbc:ID',
    mls_relevance: 'medium',
    vat_law_status: 'Conditional',
    data_responsibility: 'ERP (Corner 1)',
  },
  {
    dr_id: 'IBT-026',
    business_term: 'Preceding invoice issue date',
    category: 'conditional_header',
    mandatory_flag_by_use_case: 'Optional (Credit note preceding invoice context)',
    pint_ae_cardinality: '0..1',
    data_type: 'Date',
    format_pattern: 'ISO 8601 YYYY-MM-DD',
    validation_logic: 'If provided, must be a valid preceding invoice issue date in ISO format.',
    derivation_logic: 'Provided by ERP input; no ASP derivation.',
    error_message_text: 'Preceding invoice issue date must use YYYY-MM-DD format when supplied.',
    ubl_xml_path: 'cac:BillingReference/cac:InvoiceDocumentReference/cbc:IssueDate',
    mls_relevance: 'low',
    vat_law_status: 'Conditional',
    data_responsibility: 'ERP (Corner 1)',
  },
