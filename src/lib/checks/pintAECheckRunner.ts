...
type CheckExecutor = (context: ExecutorContext) => PintAEException[];

const executePresence: CheckExecutor = ({ check, data, params, createException }) => {
...
const executePattern: CheckExecutor = ({ check, data, params, createException }) => {
  if (!params.field || !params.pattern) return [];
  ...
  dataset.forEach((record: any) => {
    const value = getFieldValue(record, field);
    if (isEmpty(value) || regex.test(String(value))) return;

    const header = record.invoice_id ? data.headerMap.get(record.invoice_id) : undefined;
    const invoiceNumber = record.invoice_number || header?.invoice_number || record.invoice_id;
    ...
    exceptions.push(
      createException({
        ...
        message: `Invoice ${invoiceNumber}: Field "${field}" format invalid. Expected pattern: ${params.pattern}`,
      })
    );
  });
...
const executeCodeList: CheckExecutor = ({ check, data, params, createException }) => {
  if (!params.field || !params.codelist) return [];
  ...
  dataset.forEach((record: any) => {
    const value = getFieldValue(record, field);
    if (isEmpty(value) || isCodeInCodelist(String(params.codelist), String(value))) return;
...
const executeHeaderPresenceByParamField: CheckExecutor = ({ data, params, createException }) => {
  const exceptions: PintAEException[] = [];
...
const executeHeaderPatternByParamField: CheckExecutor = ({ data, params, createException }) => {
  if (!params.field || !params.pattern) return [];
...
const executeUaeUc1Chk006CurrencyCodeList: CheckExecutor = ({ data, params, createException }) => {
  const exceptions: PintAEException[] = [];

  data.headers.forEach((header) => {
    const field = params.field || 'currency';
    const value = getFieldValue(header, field);
    if (isEmpty(value) || isCodeInCodelist('ISO4217', String(value))) return;
...
const executeUaeUc1Chk007TaxCurrencyAed: CheckExecutor = ({ data, params, createException }) => {
  const exceptions: PintAEException[] = [];
...
const executeUaeUc1Chk008FxRequired: CheckExecutor = ({ data, params, createException }) => {
  const exceptions: PintAEException[] = [];
  ...
  data.headers.forEach((header) => {
    ...
    if (
      ...
      (fxRate === undefined || fxRate === null || Number(fxRate) <= 0)
    ) {
      exceptions.push(
        createException({
          invoiceId: header.invoice_id,
...
const executeUaeUc1Chk009DueDateDependency: CheckExecutor = ({ data, createException }) => {
  const exceptions: PintAEException[] = [];
...
const executeUaeUc1Chk011BusinessProcessAllowedValues: CheckExecutor = ({
  data,
  params,
  createException,
}) => {
...
const executeUaeUc1Chk013SellerTrnPattern: CheckExecutor = ({ data, createException }) => {
  const exceptions: PintAEException[] = [];
...
const executeUaeUc1Chk015SellerAddressFields: CheckExecutor = ({ data, params, createException }) => {
  const exceptions: PintAEException[] = [];
...
const executeUaeUc1Chk016SubdivisionAllowedValues: CheckExecutor = ({
  data,
  params,
  createException,
}) => {
...
const executeUaeUc1Chk017BuyerNamePresent: CheckExecutor = ({ data, createException }) => {
  const exceptions: PintAEException[] = [];

  data.buyers.forEach((buyer) => {
    if (!isEmpty(buyer.buyer_name)) return;
...
const executeUaeUc1Chk018BuyerTrnPattern: CheckExecutor = ({ data, createException }) => {
  const exceptions: PintAEException[] = [];
...
const executeUaeUc1Chk020BuyerAddressFields: CheckExecutor = ({ data, params, createException }) => {
  const exceptions: PintAEException[] = [];

  data.buyers.forEach((buyer) => {
    const fields: string[] = Array.isArray(params.fields)
      ? params.fields
      : ['buyer_address', 'buyer_country'];
...
const executeUaeUc1Chk021LineSumMatchesHeader: CheckExecutor = ({ data, params, createException }) => {
  const exceptions: PintAEException[] = [];

  data.headers.forEach((header) => {
    const invoiceLines = data.linesByInvoice.get(header.invoice_id) || [];
    const lineSum = invoiceLines.reduce((sum, line) => sum + (line.line_total_excl_vat || 0), 0);
...
const executeUaeUc1Chk022023024026DecimalPrecision: CheckExecutor = ({
  data,
  params,
  createException,
}) => {
  if (!params.field || params.max_decimals === undefined) return [];
...
const executeUaeUc1Chk025TotalsFormula: CheckExecutor = ({ data, params, createException }) => {
  const exceptions: PintAEException[] = [];

  data.headers.forEach((header) => {
    if (
      header.total_incl_vat === undefined ||
...
const executeUaeUc1Chk027TaxBreakdownPresent: CheckExecutor = ({ data, createException }) => {
  const exceptions: PintAEException[] = [];
  ...
  data.headers.forEach((header) => {
    ...
    const hasLineBreakdown = invoiceLines.some(
      (line) => !isEmpty(line.tax_category_code) && line.vat_rate !== undefined && line.vat_rate !== null
    );
    const hasTaxableAmount =
      (header.total_excl_vat || 0) > 0 ||
      invoiceLines.some((line) => (line.line_total_excl_vat || 0) > 0);
...
const executeUaeUc1Chk028VatFormula: CheckExecutor = ({ data, params, createException }) => {
  const exceptions: PintAEException[] = [];
...
const executeUaeUc1Chk029VatTotalMatchesLines: CheckExecutor = ({ data, params, createException }) => {
  const exceptions: PintAEException[] = [];

  data.headers.forEach((header) => {
    const invoiceLines = data.linesByInvoice.get(header.invoice_id) || [];
    const taxSum = invoiceLines.reduce((sum, line) => sum + (line.vat_amount || 0), 0);
...
const executeUaeUc1Chk030InvoiceHasLines: CheckExecutor = ({ data, createException }) => {
  const exceptions: PintAEException[] = [];
...
const executeUaeUc1Chk031LineIdentifierPresent: CheckExecutor = ({ data, createException }) => {
  const exceptions: PintAEException[] = [];

  data.lines.forEach((line) => {
    const header = data.headerMap.get(line.invoice_id);
    if (!isEmpty(line.line_number)) return;
...
const executeUaeUc1Chk032QuantityPresent: CheckExecutor = ({ data, createException }) => {
  const exceptions: PintAEException[] = [];

  data.lines.forEach((line) => {
    const header = data.headerMap.get(line.invoice_id);
    if (line.quantity !== undefined && line.quantity !== null) return;
...
const executeUaeUc1Chk034LineNetFormula: CheckExecutor = ({ data, params, createException }) => {
  const exceptions: PintAEException[] = [];
...
const EXECUTORS_BY_ID: Record<string, CheckExecutor> = {
  'UAE-UC1-CHK-001': executeHeaderPresenceByParamField,
  'UAE-UC1-CHK-002': executeHeaderPresenceByParamField,
  'UAE-UC1-CHK-003': executeHeaderPatternByParamField,
  'UAE-UC1-CHK-004': executeHeaderPresenceByParamField,
  'UAE-UC1-CHK-005': executeHeaderPresenceByParamField,
  'UAE-UC1-CHK-006': executeUaeUc1Chk006CurrencyCodeList,
  'UAE-UC1-CHK-007': executeUaeUc1Chk007TaxCurrencyAed,
  'UAE-UC1-CHK-008': executeUaeUc1Chk008FxRequired,
  'UAE-UC1-CHK-009': executeUaeUc1Chk009DueDateDependency,
  'UAE-UC1-CHK-010': executeHeaderPatternByParamField,
  'UAE-UC1-CHK-011': executeUaeUc1Chk011BusinessProcessAllowedValues,
  'UAE-UC1-CHK-013': executeUaeUc1Chk013SellerTrnPattern,
  'UAE-UC1-CHK-015': executeUaeUc1Chk015SellerAddressFields,
  'UAE-UC1-CHK-016': executeUaeUc1Chk016SubdivisionAllowedValues,
  'UAE-UC1-CHK-017': executeUaeUc1Chk017BuyerNamePresent,
  'UAE-UC1-CHK-018': executeUaeUc1Chk018BuyerTrnPattern,
  'UAE-UC1-CHK-020': executeUaeUc1Chk020BuyerAddressFields,
  'UAE-UC1-CHK-021': executeUaeUc1Chk021LineSumMatchesHeader,
  'UAE-UC1-CHK-022': executeUaeUc1Chk022023024026DecimalPrecision,
  'UAE-UC1-CHK-023': executeUaeUc1Chk022023024026DecimalPrecision,
  'UAE-UC1-CHK-024': executeUaeUc1Chk022023024026DecimalPrecision,
  'UAE-UC1-CHK-025': executeUaeUc1Chk025TotalsFormula,
  'UAE-UC1-CHK-026': executeUaeUc1Chk022023024026DecimalPrecision,
  'UAE-UC1-CHK-027': executeUaeUc1Chk027TaxBreakdownPresent,
  'UAE-UC1-CHK-028': executeUaeUc1Chk028VatFormula,
  'UAE-UC1-CHK-029': executeUaeUc1Chk029VatTotalMatchesLines,
  'UAE-UC1-CHK-030': executeUaeUc1Chk030InvoiceHasLines,
  'UAE-UC1-CHK-031': executeUaeUc1Chk031LineIdentifierPresent,
  'UAE-UC1-CHK-032': executeUaeUc1Chk032QuantityPresent,
  'UAE-UC1-CHK-034': executeUaeUc1Chk034LineNetFormula,
};

const EXECUTORS_BY_RULETYPE: Record<string, CheckExecutor> = {
  Presence: executePresence,
  Pattern: executePattern,
  CodeList: executeCodeList,
};

function resolveExecutor(check: PintAECheck): CheckExecutor | undefined {
  return EXECUTORS_BY_ID[check.check_id] || EXECUTORS_BY_RULETYPE[check.rule_type];
}
...
export function runPintAECheck(check: PintAECheck, data: DataContext): PintAEException[] {
  ...
  const createException: ExceptionFactory = (opts) => ({
    id: generateId(),
    timestamp,
  ...
  const executor = resolveExecutor(check);
  if (!executor) return [];

  return executor({ check, data, params, createException });
}

export function runAllPintAEChecks(checks: PintAECheck[], data: DataContext): PintAEException[] {
  const enabledChecks = checks.filter((check) => check.is_enabled);
