...
export function exportTraceabilityReport(rows: TraceabilityRow[]): void {
  ...
  const csvRows = rows.map(row => [
    ...
    row.dataset_file || (row.systemDefaultAllowed ? 'system_default' : 'asp_derived'),
    row.systemDefaultAllowed ? 'system_default' : (row.dataResponsibility.includes('ASP') ? 'asp_derived' : 'template'),
