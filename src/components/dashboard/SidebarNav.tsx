...
const SIDEBAR_ITEMS: SidebarItem[] = [
  ...
  { label: 'Control Dashboard', path: '/settings', icon: Gauge, phase: 'reference' },
];
    ...
    <aside className="rounded-xl border border-border/70 bg-card shadow-lg p-3">
      ...
      <nav className="space-y-3">
        ...
        <div className="relative space-y-3">
          <div
            className="pointer-events-none absolute left-[1.08rem] top-[1.85rem] bottom-[2.35rem] w-px bg-border/80"
            aria-hidden="true"
          />
          <div
            className="pointer-events-none absolute left-[1.08rem] top-[1.85rem] w-px bg-primary/40"
            style={{
              height:
                activeFlowIndex >= 0
                  ? `${Math.min(activeFlowIndex, flowItems.length - 1) * 2.55}rem`
                  : '0px',
            }}
            aria-hidden="true"
          />
